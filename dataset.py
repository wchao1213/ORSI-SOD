...
class Data(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #print('name:',self.cfg.datapath+'/images/'+name[:-4]+'.jpg')
        image = cv2.imread(self.cfg.datapath+'/images/'+name[:-4]+'.jpg')[:,:,::-1].astype(np.float32)
        mask  = cv2.imread(self.cfg.datapath+'/gt/' +name[:-4]+'.png', 0).astype(np.float32)
        edge  = cv2.imread(self.cfg.datapath+'/edge/' +name[:-4]+'.png', 0).astype(np.float32)
        shape = mask.shape
